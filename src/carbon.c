...
int processExternal(s_area * echo, s_message * msg, s_carbon carbon)
{
    ...
    w_log(LL_CARBON,
          "Carbon external from area %s to program \"%s\"%s%s%s: msg from \"%s\" %s to \"%s\"%s%s",
          echo->areaName ? echo->areaName : "netmail",
          progname ? progname : "",
          carbon.reason ? " at reason \"" : "",
    ...
    {
        w_log(LL_FILE,
              "toss.c:processExternal() opened %s %s",
              fname ? "file" : "pipe",
              fname ? fname : progname);
    }
...
/*    if (rc == -1 || rc == 127) */
    ...
    {
        w_log(LL_ERR,
              "Execution of external program failed. Cmd is: \"%s\", return code %d",
              execstr,
              rc);
    }
 ...
 */
    ...
    if(!aexport)
    {
        if(msg->netMail)
        {
            rc = putMsgInArea(area, msg, 0, MSGSENT);
        }
        else
        {
    ...
    if(config->intab && reason)
    {
        recodeToInternalCharset((char *)reason);
    }
...
int carbonCopy(s_message * msg, XMSG * xmsg, s_area * echo)
{
    ...
    for(i = 0; i < config->carbonCount; i++, ++cb)
    {
        ...
        {
            continue;                                       /* if types differ the rule doesn't
                                                               apply */
        }
            ...
            case CC_OR: /* OR */
                ...
                if(result)
                {
                    ...
                    if(cb->move == 2)
                    {
                        ...
                        w_log(LL_CARBON,
                              "Carbon delete from %s %s%s%s: msg from \"%s\" %s to \"%s\"%s%s.",
                              echo->areaName ? echo->areaName : "netmail",
                              cb->reason ? " at reason \"" : "",
                              cb->reason ? cb->reason : "",
                              cb->reason ? "\"" : "",
