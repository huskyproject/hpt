...
int linkJamByCRC = 0;
static s_msginfo * findMsgId(s_msginfo * entries,
...
static char * stripDomain(char * msgid)
{
    ...
    if(parseFtnAddrZ(msgid, &addr, FTNADDR_GOOD, (const char **)(&p1)) & FTNADDR_ERROR)
    {
        return msgid;
    }

    /*  '(const char**)(&p1)' is needs for prevent warning "passing arg 4 of `parseFtnAddrZ'
...
int linkArea(s_area * area, int netMail)
{
    ...
    harea =
        MsgOpenArea((UCHAR *)area->fileName, MSGAREA_NORMAL,
    ...
    if(harea)
    {
        ...
        for(i = 1; i <= msgsNum; i++)
        {
            ...
            else
            {
                hmsg =
                    MsgOpenMsg(harea,
                               (word)((msgsNum >=
                                       MAX_INCORE) ? MOPEN_READ : (MOPEN_READ | MOPEN_WRITE)), i);
                ...
                {
                    MsgCloseMsg(hmsg);
                    w_log(LL_WARN, "msg %ld has no control information: thrown from reply chain",
                          i);
                    continue;
                }
        ...
        for(i = 0; i < msgsNum; i++)
        {
            ...
            if(curr->freeReply >= MAX_REPLY - 4 && (area->msgbType & (MSGTYPE_JAM | MSGTYPE_SDM)))
            {
                curr->freeReply--;
            ...
            {
                w_log(LL_WARN,
                      "msg %ld: replies count for msg %ld exceeds %d, rest of the replies won't be linked",
                      i + 1,
                      curr - msgs + 1,
                      MAX_REPLY);
                continue;
        ...
        for(i = 0; i < msgsNum; i++)
        {
            ...
            if(jam)
            {
                if((msgs[i].replyto != msgs[i].hdr.jamhdr->ReplyTo) ||
                   ...
                   (msgs[i].hdr.jamhdr->Reply1st && !msgs[i].freeReply) || msgs[i].relinked)
                {
                    if(!msgs[i].freeReply)
                    {
                        msgs[i].hdr.jamhdr->Reply1st = 0;
                    }
            ...
            if(msgs[i].relinked != 0 || msgs[i].replyto != msgs[i].hdr.xmsg->replyto ||
...
void linkAreas(char * name)
{
    ...
    if(name != NULL)
    {
        ...
        else
        {
            ...
            {
                w_log(LL_ERR, "area %s not found for link", name);
            }
        }
