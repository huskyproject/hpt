...
void afReportAutoCreate(char * c_area, char * descr, hs_addr pktOrigAddr, ps_addr forwardAddr)
{
    ...
    if(forwardAddr == NULL)
    {
        makeMsgToSysop(c_area, pktOrigAddr, NULL);
    }
    else
    {
        makeMsgToSysop(c_area, *forwardAddr, &pktOrigAddr);
    }
...
int afRescanArea(char ** report, s_link * link, s_area * area, long rescanCount, long rescanAfter)
{
    ...
    {
        xscatprintf(report, " %s %s  no rescan possible\r", an, repeat_char(49 - strlen(an), '.'));
    ...
    else
    {
        ...
        {
            rcc = 0;
            xscatprintf(report, " %s %s  no access to export\r", an,
...
void autoPassive(void)
{
    ...
    for(i = 0; i < config->linkCount; i++)
    {
        ...
        if(createOutboundFileName(config->links[i], config->links[i]->echoMailFlavour,
                                  FLOFILE) == 0)
        {
                    ...
                    if(*path && (*path == '^' || *path == '#'))
                    {
                           ...
                           stat(path, &stat_file) != -1)
                        {
                            ...
                            if(time_test >= (time_t)config->links[i]->autoPause * 24)
                            {
                                ...
                                if(Changepause((cfgFile) ? cfgFile : getConfigFileName(),
                                               config->links[i], 1,
                                               config->links[i]->Pause ^ (ECHOAREA | FILEAREA)))
                                {
                                    UINT j, k;
                                    int mask =
                                        config->links[i]->areafix.reportsAttr ? config->links[i]->
                                        areafix.reportsAttr : robot->reportsAttr;
                                    msg =
                                        makeMessage(config->links[i]->ourAka,
                                    ...
                                    for(k = 0; k < config->echoAreaCount; k++)
                                    {
                                        ...
                                        {
                                            if(config->links[i] ==
                                               config->echoAreas[k].downlinks[j]->link)
                                            {
                                                setLinkAccess(config,
                                                              &(config->echoAreas[k]),
                                                              config->echoAreas[k].downlinks[j]);
                                                break;
                                            }
                                        }
                                    }
