...
char * skipReSubj(char * subjstr)
{
    ...
    if((ptr = skipReSubj(subjstr)) != NULL)
    {
        subjstr = ptr;
    }
...
void linkMsgs(s_msginfo * crepl, s_msginfo * srepl, dword i, dword j, s_msginfo * replmap)
{
    ...
    if(maxreply == MAX_REPLY)    /*  Squish */
    {
        ...
        {
            w_log(LL_WARN,
                  "replies count for msg %ld exceeds %d, rest of the replies won't be linked",
                  (long)j,
                  maxreply);
            links_ignored++;
        }
...
static char * GetCtrlValue(char * ctl, char * kludge)
{
    ...
    if((value = strstr(ctl, kludge)) == NULL)
    {
        return NULL;
    }
...
void linkArea(s_area * area)
{
    ...
    if(harea)
    {
        ...
        {
            w_log(LL_CRIT,
                  "Out of memory: can't get %ld bytes",
        ...
        if(loglevel >= 11)
        {
            if(linkNew)
            {
                w_log(LL_LINKPASS, "Pass 2: building relations for %ld messages, new from %ld",
        ...
        {
            if(crepl->replyId || crepl->msgId || crepl->subject)
            {
                replDone = 0;
                j        = i + 1;
                srepl    = crepl + 1;
                ...
                for( ; j <= highMsg && !replDone; j++, srepl++)
                {
                    ...
                    if(!replFound && (crepl->treeId == 0 || srepl->treeId == 0) &&
                       crepl->replyId && srepl->replyId)
                    {
                        if(cmpMsgIdReply(crepl->replyId,
                                         srepl->replyId) == 0 &&
                    ...
                    if(!replFound && (srepl->treeId == 0) && crepl->subject && srepl->subject)
                    {
                    ...
                    if(replFound && singleRepl && !hardSearch)
                    {
                        replDone++;
                    }
                }
        ...
        {
            if(crepl->replyToPos == 0 && crepl->freeReply == 0 && crepl->treeId &&
               i != crepl->treeId)
            {
...
int main(int argc, char ** argv)
{
    ...
    if(argareas)
    {
        ...
        for(j = 0; j < nareas; j++)
        {
            ...
            /*  EchoAreas */
            for(i = 0, area = config->echoAreas; i < config->echoAreaCount && !found; i++, area++)
            {
            ...
            /*  Local Areas */
            for(i = 0, area = config->localAreas; i < config->localAreaCount && !found;
                i++, area++)
            {
                if(stricmp(area->areaName, argareas[j]) == 0)
                {
            ...
            /*  NetMail areas */
            for(i = 0, area = config->netMailAreas; i < config->netMailAreaCount && !found;
                i++, area++)
            {
                if(stricmp(area->areaName, argareas[j]) == 0)
                {
        ...
        if(f)
        {
            ...
            while(!feof(f))
            {
                ...
                if(line)
                {
                    ...
                    /*  EchoAreas */
                    for(i = 0, area = config->echoAreas; i < config->echoAreaCount && !found;
                        i++, area++)
                    {
    ...
    if(links_ignored)
    {
        w_log(LL_SUMMARY, "Linked total: %ld, Ignored: %ld", (long)links_total,
              (long)links_ignored);
    }
    else
    {
