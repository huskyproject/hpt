...
static void FixWildcard(void)
{
 ...
 {
  if (Node[i].idx != WILDVALUE && IsMyNode(Node[Node[i].idx].addr) &&
       !DirectLink(i))
  {
...
static void PutDownLinksGeneric(ushort UpIdx, ushort type, ushort(*wrtnode) (nodeaddr, char *, ushort))
{
 ushort i;
 for (i = 0; i < nNodes; i++)
 {
  ...
  for (i = 0; i < nNodes; i++)
  {
   ...
   if (Node[i].idx != WILDVALUE &&	// routed
	...
	(Node[Node[i].idx].idx != WILDVALUE &&
	 ...
	 DirectLink(Node[i].idx) ) && !IsMyNode(Node[Node[i].idx].addr) ||
	DirectLink(i) )
   {
    strcpy(Buff, ".");
...
ushort WriteNodeFtrack(nodeaddr Node, char *out, ushort addrtype)
{
 ...
 if (wildlevel == level || addrtype == 1)
 {
  ...
  if (wildlevel == 1 && addrtype == 0 && level == 0)
  {
   strcpy(out, prefix);
 ...
 for (level = 0; level < 4; level++)
 {
  ...
  for (i = 0; i < nLinks; i++)
  {
   ...
   if (counter)
   {
    ...
    AddPoint = false;
    tmpNode = Link[i].addr;
    if ( DirectLink(tmpNode) && MyNode->z == tmpNode.z && MyNode->n == tmpNode.n && tmpNode.f == WILDVALUE )
    {
     strcat(Buff,"%.0");
    }
    else
